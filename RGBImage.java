...
public class RGBImage implements Image{
    ...
    /*Δημιουργεί μία RGB εικόνα με διαστάσεις πλάτος width και 
    ύψος height και μέγιστη τιμή φωτεινότητας colordepth .*/
    ...
    /*Δημιουργεί μία εικόνα RGB από μία άλλη εικόνα RGB. 
    Η νέα εικόνα είναι αντίγραφο της αρχικής (copy constructor).*/
    ...
    /*Επιστρέφει έναν ακέραιο που αντιστοιχεί στην τιμή του βάθους χρώματος της εικόνας.*/
    public int getColorDepth(){
	return MAX_COLORDEPTH;
    }
    
    /*επιστρέφει το αντικείμενο τύπου Pixel που 
    αντιστοιχεί στη γραμμή row, col του πίνακα της εικόνας.*/
	...
	for(int i=0;i<height/2;i++){
		...
		red = (short)((pixels[2*i][2*j].getRed()+pixels[2*i+1][2*j].getRed()+pixels[2*i][2*j+1].getRed()+pixels[2*i+1][2*j+1].getRed())/4);
		green = (short)((pixels[2*i][2*j].getGreen()+pixels[2*i+1][2*j].getGreen()+pixels[2*i][2*j+1].getGreen()+pixels[2*i+1][2*j+1].getGreen())/4);
		blue = (short)((pixels[2*i][2*j].getBlue()+pixels[2*i+1][2*j].getBlue()+pixels[2*i][2*j+1].getBlue()+pixels[2*i+1][2*j+1].getBlue())/4);
